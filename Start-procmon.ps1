...
function cleanup
{
	...
	$ExistCSV = "False"
	if((test-path $logCSV) -eq $true)
	{
		$ExistCSV = "True"
		del $logCSV -Force
	}
	del $logPml -Force
	if((Test-Path $LogPML) -eq $true)
...
start-process -filepath ".\Procmon" -argument "/backingfile $LogPML /quiet /accepteula" `
	-WindowStyle Hidden
...
Start-Process -FilePath ".\procmon" -argument "/openlog $LogPml /saveAs $LogCSV /accepteula" `
...
$OutputObj = import-csv $LogCSV -ErrorVariable ImportCSVErr
